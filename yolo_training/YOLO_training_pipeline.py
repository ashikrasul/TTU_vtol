...
class YOLOTrainingPipeline:
    ...
    def train_yolov8(self):
        ...
        # Set scale and hsv_v in the configuration
        config['scale'] = self.scale_value
        config['hsv_v'] = self.hsv_v_value
        print(f"Setting scale to {self.scale_value} and hsv_v to {self.hsv_v_value}")

        # Save the modified configuration to a temporary file
        temp_cfg_file = os.path.join(self.save_dir, 'temp_config.yaml')
        ...
        self.cleanup(save_dir=os.path.join(self.save_dir, 'train'))
